...
The skill reads `~/.config/opencode/feishu.json`. If this file does not exist, **all Feishu functionality is disabled** — skills behave exactly as before.
...
```bash
cat ~/.config/opencode/feishu.json 2>/dev/null
```
...
Check if `~/.config/opencode/feishu.json` exists and mode is not "off":
