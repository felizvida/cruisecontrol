...
The executable probe sharpens that argument. Even though the probe is synthetic, it captures one genuinely biomolecular pressure point: a local environment can be chemically meaningful only if its mixed-channel neighborhood remains accessible after tokenization. The gap between locality-aware orderings and the raster or random baselines under tight sequence budgets suggests that interface quality should be evaluated in terms that are native to sequence processing, not only in terms of geometric nostalgia.
