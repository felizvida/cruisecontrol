...
\input{figures/TABLE_taxonomy.tex}

Table~\ref{tab:taxonomy} organizes the relevant literature into a simple taxonomy. The point of the taxonomy is not to claim that all of these models are equivalent. They are not. Rather, the taxonomy makes visible a shared design space that is easy to miss when papers are grouped only by backbone family. Section~3 showed that this shared design space is not merely philosophical: different orderings expose measurably different locality and context-budget profiles even in a small executable probe.
...
This taxonomy clarifies why the 2018 paper still matters. It occupies the earliest row in a family of methods that all ask the same underlying question: how should a structural object be ordered so that the resulting computation is both tractable and faithful enough for the task? The probe results suggest that answering that question requires more than reporting one average locality number; it requires looking at the context-budget curve an ordering creates.
